...
// PERUBAHAN: onConfirmPayment sekarang akan menerima jumlah uang yang dibayar
export const PaymentModal = ({ isOpen, onClose, totalPrice, onConfirmPayment }) => {
  ...
  const handleConfirm = () => {
    // Kirim jumlah yang dibayar ke parent component
    ...
    <div className="fixed inset-0 bg-black bg-opacity-60 z-40 flex justify-center items-center">
      <div className="bg-white rounded-lg shadow-xl p-6 w-full max-w-sm m-4">
        ...
        <div className="mb-4">
          ...
          <input type="number" name="amountPaid" id="amountPaid" value={amountPaid} onChange={(e) => setAmountPaid(e.target.value)} className="w-full text-lg p-2 border-gray-300 rounded-md shadow-sm" placeholder="Contoh: 50000" autoFocus />
        </div>
        ...
        <div className="flex flex-col gap-3">
          <button onClick={handleConfirm} disabled={!isPaymentSufficient} className="w-full bg-blue-600 text-white rounded-lg py-3 font-bold hover:bg-blue-700 transition-colors disabled:bg-gray-400">Konfirmasi Pembayaran</button>
          <button type="button" onClick={onClose} className="w-full bg-gray-200 text-gray-800 rounded-lg py-2 hover:bg-gray-300">Batal</button>
        </div>
      </div>
    </div>
  );
