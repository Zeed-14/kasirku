  ...
  useEffect(() => {
    if (product) {
      setProductData({
        name: product.name || '',
        price: product.price || '',
        category: product.category_id ? product.categories.name : (product.category || categories[0]),
        ...
        <form onSubmit={handleSubmit} className="flex-grow flex flex-col overflow-hidden">
          ...
          <div className="p-6 overflow-y-auto">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
              <div className="relative"><span className="absolute inset-y-0 left-0 flex items-center pl-3"><RupiahIcon className="h-5 w-5 text-gray-400" /></span><input type="number" name="price" placeholder="Harga" value={productData.price} onChange={handleChange} className="w-full pl-10 p-2 border border-gray-300 rounded-md" required /></div>
              <div className="relative"><span className="absolute inset-y-0 left-0 flex items-center pl-3"><StockIcon className="h-5 w-5 text-gray-400" /></span><input type="number" name="stock" placeholder="Stok" value={productData.stock} onChange={handleChange} className="w-full pl-10 p-2 border border-gray-300 rounded-md" required /></div>
            </div>
            <div className="relative mb-4"><span className="absolute inset-y-0 left-0 flex items-center pl-3"><CategoryIcon className="h-5 w-5 text-gray-400" /></span><select name="category" value={productData.category} onChange={handleChange} className="w-full pl-10 p-2 border border-gray-300 rounded-md">{categories.map(cat => (<option key={cat} value={cat}>{cat}</option>))}</select></div>
            <div className="relative"><span className="absolute inset-y-0 left-0 flex items-center pl-3"><BarcodeIcon className="h-5 w-5 text-gray-400" /></span><input type="text" name="barcode" placeholder="Barcode (opsional)" value={productData.barcode} onChange={handleChange} className="w-full pl-10 p-2 border border-gray-300 rounded-md" /></div>
          </div>
