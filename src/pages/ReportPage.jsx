...
// Impor dari folder 'common'
import { ReceiptModal } from '../components/common/ReceiptModal';
import { ReportSkeleton } from '../components/common/ReportSkeleton';

// Impor dari folder 'icons'
import { RevenueIcon, TransactionIcon, BestSellerIcon, ClockIcon } from '../components/icons';
      ...
      if (error) {
        ...
        data.forEach(tx => {
          tx.items.forEach(item => {
            ...
            itemCounts[item.name].revenue += revenue;
            const category = item.category || 'Lain-lain';
            categoryRevenue[category] = (categoryRevenue[category] || 0) + revenue;
          });
        });
        
        const sortedBestSellers = Object.values(itemCounts).sort((a, b) => b.quantity - a.quantity).slice(0, 5);
        setBestSellers(sortedBestSellers);
        setSummary({ revenue: totalRevenue, transactions: data.length, bestSeller: sortedBestSellers.length > 0 ? sortedBestSellers[0].name : 'N/A' });
        ...
        if (filter === 'today') {
          const hourlySales = Array(24).fill(0).map((_, i) => ({ name: `${String(i).padStart(2, '0')}:00`, total: 0 }));
          data.forEach(tx => { const hour = new Date(tx.created_at).getHours(); hourlySales[hour].total += tx.final_price; });
          setChartData(hourlySales.filter(h => h.total > 0));
        } else {
          const dailySales = {};
          data.forEach(tx => { const day = new Date(tx.created_at).toLocaleDateString('id-ID', { weekday: 'short', day: '2-digit' }); dailySales[day] = (dailySales[day] || 0) + tx.final_price; });
          setChartData(Object.entries(dailySales).map(([name, total]) => ({ name, total })).reverse());
        }
      }
      setIsLoading(false);
    };
      ...
      <div className="p-4 h-full flex flex-col overflow-hidden">
          ...
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-4">
            ...
            <div className="flex items-center gap-2 mt-2 sm:mt-0 bg-white p-1 rounded-lg shadow-sm">
              {['today', 'week', 'month'].map(f => (
                <button key={f} onClick={() => setFilter(f)} className={`px-3 py-1 text-sm font-semibold rounded-md ${filter === f ? 'bg-blue-600 text-white' : 'text-gray-600 hover:bg-gray-100'}`}>
        ...
        <div className="flex-grow overflow-y-auto">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
            ...
            <ReportSummaryCard title="Jml. Transaksi" value={summary.transactions} icon={<TransactionIcon className="h-6 w-6"/>} />
            <ReportSummaryCard title="Produk Terlaris" value={summary.bestSeller} icon={<BestSellerIcon className="h-6 w-6"/>} />
            <ReportSummaryCard title="Lihat Riwayat" value="Semua" icon={<ClockIcon className="h-6 w-6"/>} onClick={() => setIsHistoryModalOpen(true)} />
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div className="lg:col-span-1 flex flex-col">
              <h2 className="text-xl font-bold mb-3">Transaksi Terbaru</h2>
              <div className="flex-grow bg-gray-100 p-3 rounded-lg overflow-y-auto h-80">
                {transactions.length > 0 ? (
                  transactions.map(tx => (
                    <TransactionListItem key={tx.id} transaction={tx} onClick={handleTransactionClick} />
                  ))
                ) : (
                  <p className="text-center text-gray-500 mt-10">Belum ada transaksi pada periode ini.</p>
                )}
              </div>
            </div>
          </div>
