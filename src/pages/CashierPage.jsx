...
import { ShoppingCartIcon, SearchIcon, BarcodeIcon, FilterIcon } from '../components/icons';
import { supabase } from '../lib/supabaseClient';

const PRODUCTS_PER_PAGE = 12;

export const CashierPage = () => {
  // State yang tidak berhubungan dengan manajemen produk (edit/delete) tetap ada
  ...
  // State untuk edit & delete dihapus
  ...
  // Fungsi handleSaveProduct tetap ada untuk alur scan barcode
  const handleSaveProduct = async (productData, imageFile) => {
    ...
    try {
      ...
      const finalProductData = { name: productData.name, price: parseFloat(productData.price), category_id: productData.category, barcode: productData.barcode, image_url: imageUrl, stock: productData.stock };
      const { data, error: insertError } = await supabase.from('products').insert([finalProductData]).select();
      if (insertError) throw insertError;
      if (data) {
        setProducts(prev => [data[0], ...prev]);
        setNewProductBarcode('');
        toast.success('Produk baru berhasil disimpan!', { id: toastId });
      }
    } catch (error) {
  ...
  // Fungsi handleUpdateProduct dan handleDeleteProduct dihapus
  ...
  const categories = useMemo(() => ['Semua', ...new Set(products.map(p => p.categories?.name).filter(Boolean))], [products]);
  ...
  return (
    <>
          ...
          <div>
            <div className="bg-white p-3 rounded-lg shadow-sm mb-4">
              <div className="flex flex-col sm:flex-row gap-3">
                ...
                {/* Tombol Tambah dihapus, hanya tombol Scan yang tersisa */}
                <div className="flex items-center gap-2">
                  <button onClick={() => setIsScannerOpen(true)} className="w-full flex items-center justify-center gap-2 bg-green-600 text-white font-semibold py-2 px-4 rounded-lg shadow hover:bg-green-700"><BarcodeIcon className="h-5 w-5" /><span>Scan</span></button>
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
                {products.length > 0 ? products.map(p => (
                  <ProductCard 
                    ...
                    // Prop onEdit dan onDelete dihapus
                  />
                )) : <div className="col-span-full text-center text-gray-500 py-10"><p>Produk tidak ditemukan.</p></div>}
              </div>
            }
      ...
      {/* Modal Edit dan Confirm dihapus dari sini */}
